...
function setUnidades() {
	$('.product-shelf__id').each(function(index) {
		...
		$('<ul class="receive-qtd"></ul>').insertAfter(myParent);
		var myReceive = $(myParent).find('.receive-qtd');
		...
		$.getJSON(data, function(data) {
			$.each(data, function(key, val) {
				...
				$(myItems).each(function(data, val){
					...
					$('<li class="my-qtd el'+qtd+'" style="display: block; text-align: center; font-size: 13px !important;">Faltam <b>'+qtd+'</b> itens desse produto.</li>').appendTo(myReceive);
				});
			});
		});
	});
