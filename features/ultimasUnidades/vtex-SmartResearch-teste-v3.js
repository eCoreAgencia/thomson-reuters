...
function setUnidades() {
    $('.product-shelf__id').each(function(index) {
        var myParent = $(this).parent().find('.product__price');
        ...
        $.getJSON(data, function(data) {
            $.each(data, function(key, val) {
                var myItems = val.items[0].sellers[0].commertialOffer;

                $(myItems).each(function(data, val){
                    var qtd = val.AvailableQuantity;
                    $('<span class="my-qtd el'+qtd+'" style="display: block; text-align: center; font-size: 13px !important;">Faltam <b>'+qtd+'</b> itens desse produto.</span>').insertAfter(myParent);
                    $('.my-qtd:not(:first)').remove();
                });
            });
        });
    });
...
jQuery.fn.vtexSmartResearch=function(opts) {
    ...
    if($("a.vejamais-vtexresearch").length == 0){
		$('<div class="products-actions"><a href="#" class="vejamais-vtexresearch btn-red btn"> + <span>Ver mais</span></a></div>').insertAfter(".pager.bottom");
	}
