...
export default class SearchForm {
	...
	getSearchResult(query) {
		...
		const endpoint = isLocalhost
			? `/product.html`
			: vtexSearchPageEndpoint(query, this.shelfId, 3);
		axios
			.get(endpoint)
			.then(data => self.appendResultList(data.data))
			.catch(error =>
				self.appendResultList(
					`<span class="no-result">Não foi encontrado nenhum resultado</span>`
				)
			);
	}
	...
	appendResultList(resultList) {
		if (!resultList)
			resultList = `<span>Não foi encontrado nenhum resultado</span>`;
		const list = $(".search-form__result-list");
